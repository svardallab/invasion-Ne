...
for ((nr=1; nr<=$repeats; nr++)); do 
    echo $nr
done | xargs -I % -P $threads bash -c "$SCRIPT_DIR/GONE $options_for_GONE $fichero ${dirtemp}/${fichero}_%"
$SCRIPT_DIR/GONEaverage $dirtemp $fichero $repeats
