...
def main(
    ...
    sigma2_per_bin = df.groupby("bin_index")["mean"].var().sort_index().values
    ...
    with pm.Model() as model:
        ...
        log_lik = -Nchrom * (
            ...
            + (sigma2_mapped + (df["mean"].values - r2_mapped) ** 2)
            / (2 * sigma2_mapped)
        )
...
if __name__ == "__main__":
    if len(sys.argv) != 11:
        print(
            "Usage: python exponential_piecewise_nuts_contig.py <ld_file> <ne2_prior_mean> <ne2_prior_sd> <ne1_prior_sd> <t0_prior_mean> <t0_prior_sd> <alpha_logfold_prior_sd> <sample_size> <seed> <output_file>"
        )
        sys.exit(1)
    ld_file = sys.argv[1]
    ne2_prior_mean = float(sys.argv[2])
    ne2_prior_sd = float(sys.argv[3])
